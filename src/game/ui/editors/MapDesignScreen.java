 ...
 * and save custom battle maps. Features tile painting, event placement (HQs,
 * forts),
...
public class MapDesignScreen extends JPanel implements Refreshable {
    ...
    private enum Tool {
        PENCIL, ERASER, FILL, PICKER, EVENT
    }

    private enum EventType {
        NONE, HQ, ARMORY, HOUSE, FORT, AERIE
    }
    ...
    private static class EventInfo implements Serializable {
        ...
        EventInfo(EventType t, int o) {
            type = t;
            owner = o;
        }
    }

    private int mapW = 32, mapH = 18;
        ...
        MapState(TileEntry[][] d, Map<Point, EventInfo> e) {
            ...
            for (int i = 0; i < d.length; i++) {
                for (int j = 0; j < d[i].length; j++) {
                    if (d[i][j] != null) {
                        this.data[i][j] = new TileEntry(d[i][j].tileId, d[i][j].flipX, d[i][j].flipY,
                                d[i][j].paletteId);
            ...
            for (Map.Entry<Point, EventInfo> entry : e.entrySet()) {
                this.events.put(new Point(entry.getKey()),
                        new EventInfo(entry.getValue().type, entry.getValue().owner));
            }
        }
    }
    ...
    private JPanel createToolbar(Main main) {
        ...
        right.add(saveBtn);
        right.add(loadBtn);
        right.add(tilesetBtn);
        right.add(gridBtn);
        right.add(importBtn);
        right.add(exportBinBtn);
    ...
    private JButton styledBtn(String text) {
        ...
        b.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
                b.setBackground(new Color(60, 50, 20));
            }
    ...
    private class TilesetViewer extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int x = 0; x <= ts.getWidth(); x += tw)
                g.drawLine(x, 0, x, ts.getHeight());
            for (int y = 0; y <= ts.getHeight(); y += th)
                g.drawLine(0, y, ts.getWidth(), y);
        ...
        public Dimension getPreferredSize() {
            ...
            if (ts != null)
                return new Dimension(ts.getWidth(), ts.getHeight());
    ...
    private void cycleGroup(int dir) {
        ...
        if (currentGroupIdx < 0)
            currentGroupIdx = paletteGroups.size() - 1;
        if (currentGroupIdx >= paletteGroups.size())
            currentGroupIdx = 0;
    ...
    private JComponent createRightPanel() {
        ...
        hqBtn.addActionListener(e -> {
            selectedEventType = EventType.HQ;
            updateStatus();
        });
        JButton armoryBtn = eventBtn.apply("ARMORY");
        armoryBtn.addActionListener(e -> {
            selectedEventType = EventType.ARMORY;
            updateStatus();
        });
        JButton houseBtn = eventBtn.apply("HOUSE");
        houseBtn.addActionListener(e -> {
            selectedEventType = EventType.HOUSE;
            updateStatus();
        });
        JButton fortBtn = eventBtn.apply("FORT");
        fortBtn.addActionListener(e -> {
            selectedEventType = EventType.FORT;
            updateStatus();
        });
        JButton aerieBtn = eventBtn.apply("AERIE");
        aerieBtn.addActionListener(e -> {
            selectedEventType = EventType.AERIE;
            updateStatus();
        });
        ...
        String[] pChoices = { "P0", "P1", "P2", "P3", "P4", "P5", "P6" };
        JComboBox<String> ownerBox = new JComboBox<>(pChoices);
        styleInput(ownerBox);
        ownerBox.addActionListener(e -> {
            int idx = ownerBox.getSelectedIndex();
            selectedEventOwner = (idx >= 0) ? idx - 1 : 0;
        });
        ...
        styleInput(uTypeBox);
        uTypeBox.addActionListener(e -> {
            terrainUnitType = (String) uTypeBox.getSelectedItem();
            canvasPanel.repaint();
        });
        p.add(uTypeBox);
            ...
            private void handle(MouseEvent e) {
                ...
                if (currentTool == Tool.EVENT) {
                    if (SwingUtilities.isLeftMouseButton(e)) {
                        ...
                        if (existing == null || existing.type != selectedEventType
                                || existing.owner != selectedEventOwner) {
                            pushUndo();
                ...
                if (SwingUtilities.isLeftMouseButton(e)) {
                    if (currentTool == Tool.PENCIL) {
                        if (mapData[ty][tx].tileId != selectedTileId
                                || !mapData[ty][tx].tilesetName.equals(selectedTilesetName)) {
                            pushUndo();
    ...
    private void floodFill(int x, int y, int target, int replacement, String newTilesetName) {
        if (target == replacement && mapData[y][x].tilesetName.equals(newTilesetName))
            return;
        if (x < 0 || x >= mapW || y < 0 || y >= mapH)
            return;
        if (mapData[y][x].tileId != target)
            return;
    ...
    private void drawCanvas(Graphics2D g) {
        ...
        for (int y = 0; y < mapH; y++) {
            for (int x = 0; x < mapW; x++) {
                ...
                if (ev != null)
                    drawEventFlag(g, x * TILE_RENDER, y * TILE_RENDER, ev);
                ...
                if (showMovable) {
                    if (entry != null && entry.tileId != 0) {
                        ...
                        if (cost == -1) {
                            ...
                            g.drawLine(x * TILE_RENDER + 8, y * TILE_RENDER + 8, (x + 1) * TILE_RENDER - 8,
                                    (y + 1) * TILE_RENDER - 8);
                            g.drawLine((x + 1) * TILE_RENDER - 8, y * TILE_RENDER + 8, x * TILE_RENDER + 8,
                                    (y + 1) * TILE_RENDER - 8);
                        } else {
        ...
        if (showGrid) {
            ...
            for (int x = 0; x <= mapW; x++)
                g.drawLine(x * TILE_RENDER, 0, x * TILE_RENDER, mapH * TILE_RENDER);
            for (int y = 0; y <= mapH; y++)
                g.drawLine(0, y * TILE_RENDER, mapW * TILE_RENDER, y * TILE_RENDER);
    ...
    private void showNewMapDialog() {
        ...
        JSpinner wSpin = new JSpinner(new SpinnerNumberModel(32, 5, 100, 1));
        JSpinner hSpin = new JSpinner(new SpinnerNumberModel(18, 5, 100, 1));
    ...
    private void showResizeDialog() {
        ...
        if (r == JOptionPane.OK_OPTION) {
            ...
            for (int y = 0; y < nh; y++) {
                for (int x = 0; x < nw; x++) {
                    if (y < mapH && x < mapW)
                        newData[y][x] = mapData[y][x];
                    else
                        newData[y][x] = new TileEntry(0);
                }
            }
            mapW = nw;
            mapH = nh;
    ...
    private void updateSizeBtn() {
        ...
        for (Component c : comps) {
            if (c instanceof JPanel) {
                ...
                if (sb != null)
                    sb.setText("Resize Map: " + mapW + "x" + mapH);
            }
        }
    }
        ...
        if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try (PrintWriter pw = new PrintWriter(fc.getSelectedFile())) {
                ...
                for (int i = 0; i < keys.size(); i++) {
                    ...
                    pw.print("    {\"x\":" + p.x + ", \"y\":" + p.y + ", \"type\":\"" + ev.type + "\", \"owner\":"
                            + ev.owner + "}");
                    if (i < keys.size() - 1)
                        pw.println(",");
                    else
        ...
        if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                try {
                    ...
                    if (!paletteGroups.contains(tsName))
                        paletteGroups.add(0, tsName);
        ...
        if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                if (content.startsWith("{")) {
                    ...
                    if (dataStart == -1)
                        throw new Exception("No map data found in JSON.");
    ...
    private void importMapImage() {
        ...
        if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                importPanel.add(new JLabel("Tile Size:"));
                importPanel.add(sizeSpin);
                importPanel.add(new JLabel("X Offset:"));
                importPanel.add(xOffSpin);
                importPanel.add(new JLabel("Y Offset:"));
                importPanel.add(yOffSpin);
                importPanel.add(new JLabel("Resize Map?"));
                JCheckBox resizeCheck = new JCheckBox("", true);
                importPanel.add(resizeCheck);

                int result = JOptionPane.showConfirmDialog(this, importPanel, "Import Settings",

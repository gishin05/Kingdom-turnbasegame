# Build Desert Tileset Editor HTML
# Reads the Desert.png base64 and injects it into the HTML template

$b64 = [Convert]::ToBase64String([IO.File]::ReadAllBytes("$PSScriptRoot\..\..\assets\data\tilesets\Desert.png"))
...
<head>
    ...
    <title>Desert Tileset Editor</title>
    <meta name="description" content="Interactive terrain property editor for the Desert tileset (16x16 tiles)">
    ...
    <style>
        :root {
            ...
            --bg-card: #231a0e;
            --bg-cell: #2e2010;
            --bg-cell-hover: #3d2c18;
            --bg-cell-filled: #2a2a10;
            --border: #4a3520;
            --border-gold: #e8a030;
            --border-gold-dim: rgba(232, 160, 48, 0.25);
            --text: #f0e0c8;
            --text-dim: #a89070;
            --text-gold: #e8a030;
            --accent-green: #8abe50;
        ...
        * {
        ...
        /* ── HEADER ── */
        .header {
            background: linear-gradient(135deg, #1a0e04 0%, #2e1a08 50%, #1a1008 100%);
        ...
        .btn:hover {
            ...
            box-shadow: 0 0 12px rgba(232, 160, 48, 0.2);
        }

        .btn-primary {
            background: linear-gradient(135deg, #a06820, #d4a040);
            color: #1a0e04;
            border-color: var(--border-gold);
        }

        .btn-primary:hover {
            background: linear-gradient(135deg, #c08030, #e8b050);
            box-shadow: 0 0 20px rgba(232, 160, 48, 0.35);
        }
        ...
        .btn-success {
            background: linear-gradient(135deg, #486818, #6a9830);
            color: #fff;
            border-color: #6a9830;
        }

        .btn-success:hover {
            background: linear-gradient(135deg, #587828, #8abe50);
            box-shadow: 0 0 20px rgba(106, 152, 48, 0.3);
        }
        ...
        .tile-cell.has-data {
            ...
            border-color: rgba(138, 190, 80, 0.3);
        }
        ...
        .modal {
            ...
            min-width: 420px;
            max-width: 520px;
            box-shadow: 0 24px 80px rgba(0, 0, 0, 0.6), 0 0 40px rgba(232, 160, 48, 0.06);
        ...
        .form-group input:focus, .form-group select:focus {
            border-color: var(--border-gold);
            box-shadow: 0 0 0 3px rgba(232, 160, 48, 0.08);
        }
        ...
        ::-webkit-scrollbar-thumb:hover { background: #5a4530; }
...
<body>
    ...
    <div class="header">
        <h1>
            🏜 DESERT TILESET EDITOR
            <span>— Desert.png (16×16 tiles)</span>
        </h1>
    ...
    <div class="grid-container" id="gridContainer">
        <div class="loading">
            ...
            <span style="color: var(--text-dim);">Loading desert tileset...</span>
        </div>
    </div>
        ...
        <div class="modal">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="inputName" placeholder="e.g. SAND, DUNES, OASIS..." autocomplete="off">
                </div>
                <div class="form-row">
                    <div class="form-group">
    ...
    <div class="export-panel" id="exportPanel">
        <div class="export-content">
            <h2>📦 Exported JSON — Desert.json</h2>
            <p style="color: var(--text-dim); margin-bottom: 12px;">Copy this JSON and save it as <code>Desert.json</code> in your tilesets data folder.</p>
            <textarea class="export-textarea" id="exportTextarea" readonly></textarea>
            <div class="export-actions">
                <button class="btn" onclick="closeExport()">Close</button>
                <button class="btn btn-success" onclick="downloadExport()">💾 Download Desert.json</button>
    ...
    <div class="import-panel" id="importPanel">
        <div class="export-content">
            <h2>📥 Import JSON — Desert.json</h2>
            <p style="color: var(--text-dim); margin-bottom: 12px;">Paste your existing Desert.json data below to load it into the editor.</p>
            <textarea class="export-textarea" id="importTextarea" placeholder='{ "tiles": { "0": { "type": "SAND", ... } } }'></textarea>
            <div class="export-actions">
    ...
    <script>
        ...
        const TILE_W = 16;
        const TILE_H = 16;
        const STORAGE_KEY = 'tileset_editor_desert';

        // ── Desert-specific terrain presets ──
        const PRESETS = [
            { name: 'SAND',    land: 1,  ocean: -1, air: 1, def: 0, avo: 0 },
            { name: 'DUNES',   land: 2,  ocean: -1, air: 1, def: 0, avo: 10 },
            { name: 'OASIS',   land: -1, ocean: 1,  air: 1, def: 0, avo: 5 },
            { name: 'CANYON',  land: -1, ocean: -1, air: 1, def: 2, avo: 0 },
            { name: 'RUINS',   land: 1,  ocean: -1, air: 1, def: 1, avo: 5 },
            ...
            { name: 'ROCK',    land: -1, ocean: -1, air: -1, def: 0, avo: 0 },
            { name: 'PALM',    land: 1,  ocean: -1, air: 1, def: 0, avo: 10 },
...
Set-Content -Path "$PSScriptRoot\desert-editor.html" -Value $html -Encoding UTF8
Write-Host "Desert Tileset Editor built: $PSScriptRoot\desert-editor.html"

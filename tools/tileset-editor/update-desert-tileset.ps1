param(
  # Path to an exported JSON file from the tileset editor.
  [Parameter(Mandatory=$true)][string]$InputJson,

  # Output target; defaults to the game's Desert.json.
  [string]$OutputJson = "",
...
function Format-TileLine {
  ...
  if ([string]::IsNullOrWhiteSpace($type)) { $type = "SAND" }
...
if ([string]::IsNullOrWhiteSpace($OutputJson)) {
  $OutputJson = Join-Path $scriptDir "..\..\assets\data\tilesets\Desert.json"
}

$repoRoot = (Resolve-Path (Join-Path $scriptDir "..\..")).Path
$mapsRoot = (Join-Path $repoRoot "assets\data\maps")
$desertPng = (Join-Path $repoRoot "assets\data\tilesets\Desert.png")
...
$maxPngTiles = Get-MaxTileIdFromPng -PngPath $desertPng -TileWidth 16 -TileHeight 16
...
Write-Host "Desert.png tile capacity: $maxPngTiles"

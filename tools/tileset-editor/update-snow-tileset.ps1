param(
  ...
  # Output target; defaults to the game's Snow.json.
  [string]$OutputJson = "",
...
function Format-TileLine {
  ...
  if ([string]::IsNullOrWhiteSpace($type)) { $type = "SNOW" }
...
if ([string]::IsNullOrWhiteSpace($OutputJson)) {
  $OutputJson = Join-Path $scriptDir "..\..\assets\data\tilesets\Snow.json"
}

$repoRoot = (Resolve-Path (Join-Path $scriptDir "..\..")).Path
$mapsRoot = (Join-Path $repoRoot "assets\data\maps")
$snowPng = (Join-Path $repoRoot "assets\data\tilesets\Snow.png")
...
$maxPngTiles = Get-MaxTileIdFromPng -PngPath $snowPng -TileWidth 16 -TileHeight 16
...
Write-Host "Snow.png tile capacity: $maxPngTiles"
